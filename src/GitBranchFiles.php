 ...
 */
class GitBranchFiles extends \ArrayObject
{

    public function __construct($branch = 'HEAD:')
    {
        parent::__construct();

        $branch = preg_replace('#(?<![:/])$#', '/', $branch);
        exec('git ls-tree -r --name-only '.escapeshellarg($branch), $output);
        foreach ($output as $filename) {
            $this[$filename] = new GitFileInfo($branch . $filename);
        }
    }
}
